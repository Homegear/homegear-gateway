                    ...
                    else
                    {
                        ...
                        if(!packet.empty())
                        {
                            ...
                            else
                            {
                                ...
                                parameters->push_back(std::make_shared<BaseLib::Variable>(HOMEMATIC_CC1101_FAMILY_ID));
...
void HomeMaticCc1101::readwrite(std::vector<uint8_t>& data)
{
    try
    {
...
std::vector<uint8_t> HomeMaticCc1101::readRegisters(Registers::Enum startAddress, uint8_t count)
{
    try
    {
...
void HomeMaticCc1101::writeRegisters(Registers::Enum startAddress, std::vector<uint8_t>& values)
{
    try
    {
        if(_fileDescriptor->descriptor == -1) return;
...
BaseLib::PVariable HomeMaticCc1101::sendPacket(BaseLib::PArray& parameters)
{
    try
    {
        ...
        if(decodedPacket.empty() || decodedPacket[0] != decodedPacket.size() - 1) return BaseLib::Variable::createError(-1, "Invalid packet.");
